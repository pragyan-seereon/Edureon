...
// import { Eye, FileCheck2, FileUp, Loader2, Pencil, Trash2, X } from "lucide-react";
...
//     parentIncome: record.annual_income || "",
...
//   // user can resume even if they close the browser tab.
//   useEffect(() => {
//     if (draftUuid) {
...
//   // ── Dialog open/close + Edit vs Create vs Resume-draft entry point ──
//   useEffect(() => {
//     if (!open) return;
...
//     // ── CREATE STUDENT ──
//     // Check for a draft left behind from a previous session before
//     // showing a blank form.
//     const savedUuid = localStorage.getItem(DRAFT_STORAGE_KEY);
//     if (savedUuid) {
//       checkExistingDraft(savedUuid);
//     } else {
//       resetToBlankForm();
//     }
...
//   // Verifies the saved draft_uuid still exists on the server (via the only
//   // draft API we have — GET /student-drafts/{draft_uuid}) before offering
//   // to resume it. If it's gone/expired, silently fall back to a blank form.
//   const checkExistingDraft = async (savedUuid) => {
//     if (!instituteUUID) {
//       // Can't call the API without institute context — just start fresh.
//       resetToBlankForm();
//       return;
//     }
//     setCheckingDraft(true);
//     try {
//       const res = await getStudentDraft(savedUuid, instituteUUID);
//       const draftData = res?.data?.data ?? res?.data;
//       if (!draftData) throw new Error("Empty draft response");
//       setPendingDraft({ ...draftData, draft_uuid: draftData.draft_uuid || savedUuid });
//       setShowResumePrompt(true);
//     } catch (err) {
//       console.error(err);
//       // Stale/expired/deleted draft — clear the dangling reference.
...
//     if (pendingDraft) {
//       setF(mapRecordToForm(pendingDraft));
//       setUploaded(mapRecordToDocs(pendingDraft));
//       setDraftUuid(pendingDraft.draft_uuid);
//       // Resume on whichever tab the draft says it last reached, defaulting
//       // to the first tab if the backend doesn't track that yet.
//       setTab(
//         TAB_ORDER.includes(pendingDraft.current_step) ? pendingDraft.current_step : "personal",
//       );
//     }
//     setErrors({});
//     setShowResumePrompt(false);
//     toast.success("Resumed your saved draft");
//   };

//   const handleStartNew = async () => {
//     const staleUuid = pendingDraft?.draft_uuid || localStorage.getItem(DRAFT_STORAGE_KEY);
//     localStorage.removeItem(DRAFT_STORAGE_KEY);
//     resetToBlankForm();

//     // Best-effort cleanup of the abandoned draft on the server. Safe to
//     // skip silently if the endpoint isn't available yet.
//     if (staleUuid && typeof deleteStudentDraft === "function") {
//       try {
...
//   // user stops typing, so a draft is never lost. Skipped while the resume
//   // dialog is up, while a save is already inflight, and on the Review tab
//   // (nothing there to save), and in Edit-Student mode (explicit Save is
//   // used there instead, to avoid surprising partial writes to a live
//   // student record).
//   const autosaveTimerRef = useRef(null);
//   useEffect(() => {
//     if (!open || student) return;
//     if (showResumePrompt || checkingDraft) return;
...
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [f, tab, open, student, showResumePrompt, checkingDraft]);
...
//           // The resume-draft prompt and document viewer are portaled
//           // straight to document.body (outside Radix's own internal
//           // Portal for DialogContent), so Radix's DismissableLayer sees
//           // clicks on them as "outside" this DialogContent and would
//           // otherwise auto-close the whole Dialog. Block that while
//           // either of those custom overlays is open — they manage their
//           // own closing explicitly (Start New / Resume Draft / X button /
//           // backdrop click).
//           if (showResumePrompt || viewingDoc) {
//             e.preventDefault();
//           }
//         }}
//         onInteractOutside={(e) => {
...
//             {!student && draftUuid && !checkingDraft && (
...
//             <p className="text-sm">Checking for a saved draft…</p>
//           </div>
//         ) : (
//           <>
...
//               {/* ── REVIEW (new, always right before submit) ── */}
...
//                   <ReviewRow label="Father / Guardian" value={f.parent} />
//                   <ReviewRow label="Mother's Name" value={f.motherName} />
//                   <ReviewRow label="Primary Phone" value={f.phone} />
//                   <ReviewRow label="Email" value={f.email} />
//                   <ReviewRow label="Annual Income" value={f.parentIncome} />
//                   <ReviewRow label="Emergency Contact" value={f.emergencyContact} />
//                   <ReviewRow label="Address" value={[f.address, f.city, f.state, f.pin].filter(Boolean).join(", ")} />
//                 </ReviewSection>
...
//       {/* ── Resume Draft / Create New prompt ──
//           Rendered via createPortal directly under document.body.
//           Radix's Dialog sets `pointer-events: none` on <body> while open
//           and only re-enables it on its own portaled content, so any
//           plain `fixed` div rendered as a *child of DialogContent's tree*
//           silently inherits `pointer-events: none` and becomes unclickable.
...
//               <h3 className="font-display text-lg font-semibold">Resume your admission draft?</h3>
//               <p className="text-sm text-muted-foreground">
//                 We found an in-progress admission
//                 {pendingDraft?.full_name ? (
//                   <>
//                     {" "}
//                     for <strong>{pendingDraft.full_name}</strong>
//                   </>
//                 ) : null}
//                 . Continue where you left off, or start a fresh admission instead.
//               </p>
//               <div className="flex justify-end gap-2 pt-2">
//                 <Button variant="outline" onClick={handleStartNew}>
//                   Start New
//                 </Button>
//                 <Button className="gradient-primary border-0" onClick={handleResumeDraft}>
//                   Resume Draft
//                 </Button>
//               </div>
//             </div>
...
// // Rendered via createPortal directly under document.body (see the
// // Resume-Draft prompt comment above for why: it keeps this modal outside
// // Radix's Dialog subtree so it never inherits `pointer-events: none`
// // from <body>, and its buttons/iframe/close button stay clickable).
