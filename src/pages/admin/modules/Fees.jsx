                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => setOpenStudentId(r.student_uuid)}>
                            <Eye className="h-4 w-4" />Student Ledger
                          </DropdownMenuItem>
  <DropdownMenuContent align="end">

  <DropdownMenuItem
    onClick={() => setOpenStudentId(r.student_uuid)}
  >
...
//   const activeDef = REPORT_DEFS.find((r) => r.key === openR);
...
//     setReportError("");
//     try {
//       const res = await callReportApi(key, { fromDate, toDate, reportDate, studentUuid });
//       const { rows: extractedRows, totals: extractedTotals } = extractReport(key, res, { students });
...
//   useEffect(() => {
//     if (!openR) return;
//     if (activeDef?.filter === "student" && !studentUuid) {
//       setRows([]);
//       setTotals([]);
//       return;
//     }
...
//   /* ---------------- Custom Reports (ad-hoc builder — no dedicated    */
//   /*  backend endpoint exists for arbitrary filters/columns, so it     */
//   /*  still composes from data already loaded on the page)             */
...
//       out = ledger.filter((e) => inRange(e.date));
//       if (r.source === "cashbook") out = out.filter((e) => e.mode === "Cash");
//       if (r.source === "late") out = out.filter((e) => e.lateFee > 0);
//       if (r.source === "discount") out = out.filter((e) => e.discount > 0);
//       if (f.kind) out = out.filter((e) => e.kind === f.kind);
//       if (f.mode) out = out.filter((e) => e.mode === f.mode);
//       if (f.class) out = out.filter((e) => e.class_name === f.class);
//       if (f.minAmount) out = out.filter((e) => e.amount >= f.minAmount);
//       out = out.map((e) => ({ id: e.id, kind: e.kind, student: e.student_name, class: e.class_name, mode: e.mode ?? "", amount: e.amount, discount: e.discount, lateFee: e.lateFee, date: e.date, status: e.status }));
...
//       out = students
//         .filter((s) => !f.class || s.class_name === f.class)
//         .map((s) => ({ id: s.student_uuid, name: s.full_name, class: s.class_name, section: s.section_name }));
//     }
//     if (r.columns.length) out = out.map((row) => Object.fromEntries(r.columns.map((c) => [c, row[c]])));
//     return out;
//   };
...
//         {REPORT_DEFS.map((r) => (
//           <Card key={r.key} className="border-border/60 cursor-pointer hover:border-primary/40 transition-colors" onClick={() => openReport(r.key)}>
//             <CardHeader className="pb-2">
...
//               <Button size="sm" variant="outline" onClick={(e) => { e.stopPropagation(); openReport(r.key); }}><Eye className="h-3.5 w-3.5" />Open</Button>
//             </CardContent>
//           </Card>
//         ))}
//       </div>

//       {/* Live, API-backed report sheet */}
//       <Sheet open={!!openR} onOpenChange={(v) => !v && setOpenR(null)}>
//         <SheetContent className="w-full sm:max-w-3xl overflow-y-auto">
//           <SheetHeader>
//             <SheetTitle>{activeDef?.title}</SheetTitle>
//             <SheetDescription>{activeDef?.desc}</SheetDescription>
//           </SheetHeader>

//           <div className="py-4 space-y-3">
//             {/* Filters */}
//             <div className="flex items-end gap-2 flex-wrap">
//               {activeDef?.filter === "dateRange" && (
//                 <>
//                   <FF label="From Date"><Input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} className="h-9" /></FF>
//                   <FF label="To Date"><Input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} className="h-9" /></FF>
//                   <Button size="sm" variant="outline" onClick={() => runReport(openR)} disabled={loadingReport}>
//                     <RefreshCcw className="h-4 w-4" />Apply
//                   </Button>
//                 </>
//               )}

//               {activeDef?.filter === "reportDate" && (
//                 <>
//                   <FF label="Report Date"><Input type="date" value={reportDate} onChange={(e) => setReportDate(e.target.value)} className="h-9" /></FF>
//                   <Button size="sm" variant="outline" onClick={() => runReport(openR)} disabled={loadingReport}>
//                     <RefreshCcw className="h-4 w-4" />Apply
//                   </Button>
//                 </>
...
//               {activeDef?.filter === "student" && (
//                 <div className="w-full space-y-1.5 relative">
//                   <Label className="text-xs text-muted-foreground">Student</Label>
//                   <Input
//                     placeholder="Search by name or admission no..."
//                     value={studentUuid ? (students.find((s) => s.student_uuid === studentUuid)?.full_name ?? "") : studentQuery}
//                     onChange={(e) => { setStudentUuid(""); setStudentQuery(e.target.value); }}
//                     className="h-9"
//                   />
//                   {!studentUuid && matchingStudents.length > 0 && (
...
//                           onClick={() => { setStudentUuid(s.student_uuid); setStudentQuery(""); }}
//                         >
...
//             {activeDef?.filter === "student" && !studentUuid && (
//               <div className="text-xs text-muted-foreground border rounded-md py-6 text-center">
//                 Search and pick a student above to load their ledger.
//               </div>
//             )}

//             {reportError && (
//               <div className="text-xs text-destructive border border-destructive/30 rounded-md px-3 py-2">{reportError}</div>
//             )}

//             {totals.length > 0 && (
//               <div className="flex flex-wrap gap-2">
//                 {totals.map((t) => (
//                   <div key={t.label} className="rounded-md border border-border/60 bg-muted/30 px-3 py-1.5 text-xs">
...
//                 <Table>
//                   <TableHeader>
//                     <TableRow>{keys.map((k) => <TableHead key={k} className="capitalize whitespace-nowrap">{k.replace(/_/g, " ")}</TableHead>)}</TableRow>
//                   </TableHeader>
//                   <TableBody>
//                     {loadingReport && (
//                       <TableRow><TableCell colSpan={keys.length || 1} className="text-center py-8 text-sm text-muted-foreground">Loading…</TableCell></TableRow>
//                     )}
//                     {!loadingReport && rows.map((r, i) => (
//                       <TableRow key={i}>
//                         {keys.map((k) => (
//                           <TableCell key={k} className="text-sm whitespace-nowrap">{formatCell(k, r[k])}</TableCell>
//                         ))}
//                       </TableRow>
//                     ))}
//                     {!loadingReport && !reportError && rows.length === 0 && (
//                       <TableRow><TableCell colSpan={keys.length || 1} className="text-center py-8 text-sm text-muted-foreground">No data.</TableCell></TableRow>
//                     )}
//                   </TableBody>
//                 </Table>
//               </div>
...
//           </SheetHeader>
//           {openCustom && (() => {
//             const customRows = customData(openCustom);
//             const customKeys = customRows[0] ? Object.keys(customRows[0]) : (openCustom.columns.length ? openCustom.columns : []);
//             return (
//               <div className="py-4 space-y-3">
//                 <div className="flex gap-2 flex-wrap">
//                   <Button size="sm" variant="outline" onClick={() => exportRowsCsv(customRows, `${openCustom.name}.csv`)}><Download className="h-4 w-4" />Export CSV</Button>
//                   <Button size="sm" variant="outline" onClick={() => window.print()}><Printer className="h-4 w-4" />Print</Button>
//                   <Badge variant="outline">{customRows.length} rows</Badge>
//                 </div>
//                 <div className="border rounded-md overflow-x-auto max-h-[70vh]">
//                   <Table>
//                     <TableHeader><TableRow>{customKeys.map((k) => <TableHead key={k} className="capitalize">{k}</TableHead>)}</TableRow></TableHeader>
//                     <TableBody>
//                       {customRows.map((r, i) => (
//                         <TableRow key={i}>
//                           {customKeys.map((k) => (
//                             <TableCell key={k} className="text-sm">{formatCell(k, r[k])}</TableCell>
//                           ))}
//                         </TableRow>
//                       ))}
//                       {customRows.length === 0 && <TableRow><TableCell colSpan={customKeys.length || 1} className="text-center py-8 text-sm text-muted-foreground">No matching data.</TableCell></TableRow>}
//                     </TableBody>
//                   </Table>
//                 </div>
//               </div>
...
/* ================================================================== */
/*  8. REPORTS — API-integrated report grid using Student Fee Report   */
/* ================================================================== */
